...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!type || (type !== 'blur_overlay' && type !== 'profile_checkout' && type !== 'signup_banner' && type !== 'ggb_ranking' && type !== 'fii_ranking')) {
      return NextResponse.json(
        { error: 'Tipo inválido. Deve ser "blur_overlay", "profile_checkout", "signup_banner", "ggb_ranking" ou "fii_ranking"' },
        { status: 400 }
      );
    }
