...
export function useConversionTracking() {
  ...
  const trackView = async (type: 'blur_overlay' | 'profile_checkout' | 'signup_banner' | 'ggb_ranking' | 'fii_ranking') => {
    try {
  ...
  const trackClick = async (
    type: 'blur_overlay' | 'profile_checkout' | 'signup_banner' | 'ggb_ranking' | 'fii_ranking',
    leadId?: string
  ) => {
    try {
